...
const { loadAllLessonsForTopic, getWorkshopMeta } = useLessons()
...
const allEntries = computed(() => {
  return lessons.value.map(lesson => {
    ...
    const topicProgress = progress.value[`${learning.value}:${workshop.value}`] || {}
...
function generateReport() {
  const lines = []
  const topicName = formatLangName(workshop.value)
